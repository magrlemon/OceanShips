...
bool ASoldierCharacter::IsEnemyFor(AController* TestPC) const
{
	...
	if (GetWorld()->GetGameState())
	{
		...
		if (DefGame)
		{
			bIsEnemy = DefGame->CanDealDamage();
		}
	}
...
void ASoldierCharacter::CalcCamera(float DeltaTime, struct FMinimalViewInfo& OutResult) {
	...
	case ESoldierCamreMode::E_CameraMode_First:
		...
		CameraLoc = GetTransform().TransformPosition(FVector(-10.0f, -8.0f,80.0f)) ;
		Pos = CameraLoc;
		break;
	case ESoldierCamreMode::E_CameraMode_ThirdRot:

		CameraRot = GetController()->GetControlRotation();
		CameraLoc = GetTransform().TransformPosition(FVector(0.0f, 120.0f, 80.0f));
...
float ASoldierCharacter::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	...
	AOceanBoatsGameMode* const Game = GetWorld()->GetAuthGameMode<AOceanBoatsGameMode>();
...
bool ASoldierCharacter::CanDie(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser) const
{
	if (bIsDying										// already dying
		...
		|| GetWorld()->GetAuthGameMode<AOceanBoatsGameMode>() == NULL
		|| GetWorld()->GetAuthGameMode<AOceanBoatsGameMode>()->GetMatchState() == MatchState::LeavingMap)	// level transition occurring
	{
		return false;
	}
...
bool ASoldierCharacter::Die(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser)
{
	...
	GetWorld()->GetAuthGameMode<AOceanBoatsGameMode>()->Killed(Killer, KilledPlayer, this, DamageType);
...
void ASoldierCharacter::SetRagdollPhysics()
{
	...
	if (!bInRagdoll)
	{
		...
		SetLifeSpan(1.0f);
		FString strMode = "bInRagdoll is false";
		GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor::Red, strMode);
	}
	else
	{
		FString strMode = "bInRagdoll is true";
		GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor::Red, strMode);
		SetLifeSpan(10.0f);
	}
}
...
void ASoldierCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("CameraMode", IE_Pressed, this, &ASoldierCharacter::ChangeCameraMode);

	PlayerInputComponent->BindAction("SoldierCrouch", IE_Pressed, this, &ASoldierCharacter::SoldierAnimCrouch);

	PlayerInputComponent->BindAction("SoldierProne", IE_Pressed, this, &ASoldierCharacter::SoldierAnimProne);
...
{
	UArmySimGameInstance* sGameInstance = StaticCast<UArmySimGameInstance*>(GWorld->GetGameInstance());
