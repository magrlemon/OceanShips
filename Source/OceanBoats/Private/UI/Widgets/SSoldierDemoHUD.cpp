	...
	[
		...
		[		
			...
			[
				...
				[
					...
					.BackgroundBrush(FArmySimStyle::Get().GetBrush("OceanBoats.ReplayTimelineBorder"))
					.BackgroundPadding(FMargin(0.0f, 3.0))
					.IndicatorBrush(FArmySimStyle::Get().GetBrush("OceanBoats.ReplayTimelineIndicator"))
				]
		...
		[
			...
			[
				...
				[
					SNew(SImage)
					.Image(FArmySimStyle::Get().GetBrush("OceanBoats.ReplayPauseIcon"))
				]
			]
...
FText SSoldierDemoHUD::GetPlaybackSpeed() const
{
	...
	/*if (PlayerOwner->GetWorldSettings()->GetPauserPlayerState() == nullptr)
	{
		...
		return FText::Format(NSLOCTEXT("OceanBoats.HUD.Menu", "PlaybackSpeed", "{0} X"), FText::AsNumber(PlayerOwner->GetWorldSettings()->DemoPlayTimeDilation, &FormatOptions));
	}*/

	return NSLOCTEXT("OceanBoats.HUD.Menu", "Paused", "PAUSED");
}
