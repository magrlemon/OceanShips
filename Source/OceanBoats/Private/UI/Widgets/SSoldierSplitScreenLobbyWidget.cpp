...
#define LOCTEXT_NAMESPACE "OceanBoats.SplitScreenLobby"
...
void SSoldierSplitScreenLobby::Construct( const FArguments& InArgs )
{
	...
	PressToFindText = LOCTEXT("PressToFind", "");
	PressToStartMatchText = LOCTEXT("PressToStart", "");
	PlayAsGuestText = LOCTEXT("PlayAsGuest", "<img src=\"OceanBoats.Switch.Left\"/> Play As Guest");
#endif	
	...
	const FButtonStyle* ButtonStyle = &FArmySimStyle::Get().GetWidgetStyle<FButtonStyle>("DefaultSoldierButtonStyle");
...
//								.ColorAndOpacity(MenuTitleTextColor)
							...
							[							
								SNew(SRichTextBlock)
								.TextStyle(FArmySimStyle::Get(), "OceanBoats.SplitScreenLobby.StartMatchTextStyle")							
...
{
	UArmySimGameInstance* const GameInstance = GetGameInstance();
...
{
	UArmySimGameViewportClient* SoldierViewport = Cast<UArmySimGameViewportClient>(GetGameInstance()->GetGameViewportClient());
...
{
	const UArmySimGameInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
...
UArmySimGameInstance * SSoldierSplitScreenLobby::GetGameInstance() const
{
	...
	check( PlayerOwner->GetGameInstance() == nullptr || CastChecked< UArmySimGameInstance >( PlayerOwner->GetGameInstance() ) != nullptr );

	return Cast< UArmySimGameInstance >( PlayerOwner->GetGameInstance() );
}
