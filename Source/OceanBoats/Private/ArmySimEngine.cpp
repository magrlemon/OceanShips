...
#include "ArmySimEngine.h"
#include "ArmySimGameInstance.h"

UArmySimEngine::UArmySimEngine(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UArmySimEngine::Init(IEngineLoop* InEngineLoop)
{
...
void UArmySimEngine::HandleNetworkFailure(UWorld *World, UNetDriver *NetDriver, ENetworkFailure::Type FailureType, const FString& ErrorString)
{
		...
		//if (World)
		{
			...
			if (NetDriver)
			{
				switch (FailureType)
				{
					...
					{
						UArmySimGameInstance* const ASimGameInstance = Cast<UArmySimGameInstance>(GameInstance);
						if (ASimGameInstance && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// this is the case
							ASimGameInstance->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), ArmySimGameInstanceState::MainMenu, false );
						}
						break;
					}
					case ENetworkFailure::PendingConnectionFailure:						
					{
						UArmySimGameInstance* const GI = Cast<UArmySimGameInstance>(GameInstance);
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
							// this is the case
							GI->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), ArmySimGameInstanceState::MainMenu, false );
						}
						break;
					}
					...
					case ENetworkFailure::ConnectionTimeout:
					{
						...
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
							// this is the case
							GI->ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), ArmySimGameInstanceState::MainMenu, false );
						}
						break;
					}
