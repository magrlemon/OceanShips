...
public:
		...
		m_AllocatorTag( allocatorTag )
	{
		...
		Allocator* allocator = new Allocator( ALLOC_SIZE, Allocatex( ALLOC_SIZE, allocatorTag ), sizeof( OBJECT_TYPE ), alignof(OBJECT_TYPE) );
	...
	void* CreateObject( )
	{
		...
		if (slot == nullptr) {
			Allocator* allocator = new Allocator( ALLOC_SIZE, Allocatex( ALLOC_SIZE, this->m_AllocatorTag ), sizeof( OBJECT_TYPE ), alignof(OBJECT_TYPE) );
